...
def run(params, config, capture, detector, reid):
    ...
    while thread_body.process:
        if not params.no_show:
            ...
            if key == 27 or time.time()-starttime>240:
                break
    ...
    t=time.localtime()
    output="data\\"
    output+=str(t.tm_mday)+str(t.tm_mon)+str(t.tm_year)+"\\"
    output+= str(t.tm_hour*60+t.tm_min)+".json"

    save_json_file(output, tracker.get_all_tracks_history(), description='History file')
